    ...
    stages {
        ...
        stage('Build') {
            steps {
                ...
                script {
                    // Find Java location and set JAVA_HOME (info only, actual detection in shell script)
                    def javaVersion = sh(returnStdout: true, script: 'java -version 2>&1 | head -1', ).trim()
                    echo "Java Version: ${javaVersion}"
                    
                    // For macOS, show detected Java home (info only)
                    if (isUnix()) {
                        def osType = sh(script: 'echo "$OSTYPE"', returnStdout: true).trim()
                        if (osType.contains("darwin")) {
                            def macJava = sh(script: '/usr/libexec/java_home 2>/dev/null || echo ""', returnStdout: true).trim()
                            if (macJava) {
                                echo "✅ macOS Java detected at: ${macJava}"
                            }
                        }
                    }
                    // Note: JAVA_HOME will be set in the actual build shell script for proper export
                }
                sh '''
